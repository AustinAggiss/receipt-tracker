...
export function ReceiptList({ searchQuery, dateFilter = "" }: ReceiptListProps) {
  const allReceipts = useQuery(api.receipts.list);
  // If the user provided either a text query or a date filter, call the search endpoint
  const shouldSearch = Boolean(searchQuery || dateFilter);
  const searchArgs = shouldSearch ? { query: searchQuery || "", date: dateFilter || "" } : "skip";
  const searchResults = useQuery(api.receipts.search, searchArgs);
  const receipts = shouldSearch ? searchResults : allReceipts;
